...
class TwitterAPICaller: BDBOAuth1SessionManager {    
    ...
    func getUserRequest(success: @escaping ([NSDictionary]) -> (), failure: @escaping (Error) -> ()){
        let url = "https://api.twitter.com/1.1/account/verify_credentials.json"
        
        TwitterAPICaller.client?.get(url, parameters: ["include_email"], progress: nil, success: { (task: URLSessionDataTask, response: Any?) in
            success(response as! [NSDictionary])
        }, failure: { (task: URLSessionDataTask?, error: Error) in
            failure(error)
        })
